...
const ReservationSlug = ({ reservation, item }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  const router = useRouter();
  const { id } = router.query;
  return <Reservation reservationId={id as string} reservation={{ ...reservation, itemName: item?.name ?? '' }} />;
};
...
export const getServerSideProps = (async ({ params }) => {
  try {
    ...
    const item = items.data.filter((e) => e.id === reservation.data.bookable_item)?.[0];
